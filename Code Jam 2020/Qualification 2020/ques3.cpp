...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<pair<ll,ll>> sorting(2*n);
        ll curr_1=-1,curr_2=-1;
        vector<ll>ans(n);
        FOR(i,0,n)
        {
            cin>>inp[i].first>>inp[i].second;
            sorting[2*i]={inp[i].first,i};
            sorting[2*i+1]={inp[i].second,i};
        }
        sort(sorting.begin(),sorting.end());
        bool flg=true;
        FOR(i,0,2*n)
        {
            if(done[sorting[i].second])
            {
                continue;
            }
            else
            {
                if(curr_1<=sorting[i].first)
                {
                    ans[sorting[i].second]=1;
                    curr_1=inp[sorting[i].second].second;
                    done[sorting[i].second]=true;
                }
                else if(curr_2<=sorting[i].first)
                {
                    ans[sorting[i].second]=2;
                    curr_2=inp[sorting[i].second].second;
                    done[sorting[i].second]=true;
