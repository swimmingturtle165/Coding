...
// Soln 1
// Basiclly we have to form an MST which may not be a connected Graph
// Initialize answer array as A;
...
vector<int> solve(vector<int>&A,vector<vector<int>> &B)
{
    ...
    while (flg)
    {
        ...
        {
            if((ans[B[i][1]-1]<2*B[i][2] +ans[B[i][0]-1]) || (ans[B[i][0]-1]<2*B[i][2] +ans[B[i][1]-1]))
            {
                flg=true; //becomes true only when any  optimisation  is possible
                // As A and ans are 1 based indexing i.e. ans of ith city is stored  in A[i-1]
                ans[B[i][1]-1]=min(ans[B[i][1]-1],2*B[i][2] +ans[B[i][0]-1]);
                ans[B[i][0]-1]=min(ans[B[i][0]-1],2*B[i][2] +ans[B[i][1]-1]);
            }
        }
    }
    return ans;
    //Worst Time complexity=O((B.size())^2) 
}
