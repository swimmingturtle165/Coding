...
void solv(int curr,int height,vector<vector<int>>& A,int& p,vector<bool>& vst)
{
    if(hieght<=p)
    {
        cnt++;
    ...
    {
        if(vst[A[curr][i]]==false)
        {
            solv(A[curr][i],height+1,A,p,vst);
        }
    }
}

void solve( vector<vector<int>>& A,int p)
{
    // Basically if we assign colours to all nodes which have hieght from 1-p(both inclusive)
    cnt=0;
    int n=A.size()+1;  //as A has n-1 edges
    ...
    // now we need to get count of all nodes such that their hieght is <= than p
    vector<bool> vst(n+1,false);
    solv(1,1,A,p,vst);
    return ntopowermandMod(2,cnt,1000000007); 
}
