...
#define endl '\n'         
#define ordered_set_single tree<ll,null_type,less<ll>,rb_tree_tag,tree_order_statistics_pair<ll,pll>_update>
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<vector<ll>> parent(n+1,vector<ll>(n+1,-1));
        ...
        while(q.size()>0)
        {
            ...
            for(auto it:graph[crnt])
            {
                if(strg.count({prv,{crnt,it}})==1)
                {
                    continue;
                }
                if(cost[crnt][it]>cost[prv][crnt]+1)
                {
                    cost[crnt][it]=cost[prv][crnt]+1;
                    parent[crnt][it]=prv;
                    q.push({crnt,it});
                }
            }
