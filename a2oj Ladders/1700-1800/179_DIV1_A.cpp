...
bool find(vector<ll>&Arr,int A,int B)
{
    ...
    return false;
        ...
        vector<ll> inp(100003);  


        vector<ll> strg1(100003,0);
        vector<ll> strg2(100003,0);
        vector<ll> inp1 (100003,0);
        vector<ll> strg3(100003,0);
        vector<ll> prefix_sum(100003,0);
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,n) cin>>inp[i+1];



        FOR(i,0,m)
        {
            cin>>strg1[i]>>strg2[i]>>strg3[i];
        }
        FOR(i,0,k)
        {
            ull start,end;
        ...
        // dispvector<ll>(prefix_sum);
        // dispvector<ll>(inp);

        FOR(i,0,m)
        {
            inp1[strg1[i]]+=prefix_sum[i+1]*strg3[i];
            inp1[strg2[i]+1]-=prefix_sum[i+1]*strg3[i];
            // dispvector<ll>(inp);
