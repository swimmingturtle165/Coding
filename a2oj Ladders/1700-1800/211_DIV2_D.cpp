...
signed main(int argc, char **argv)
{
    ...
    while (t--)
    {
        ...
        {
            ll mid = (maxi + mini) / 2;
            // try giving  mid cheapest cycle to mid richest kids
            // if it works mini=mid;
            // else maxi=mid-1;
            ...
            FOR(j, 0, mid)
            {
                // cout << money[j] << " " << rent[mid - 1 - j] << endl;
                // pair ith richest with (mid-1-i)th richest one
                sum += rent[j];
                if (money[j] < rent[mid - 1 - j])
                {
                    if (money[j] + grp_fund < rent[mid  - 1 - j])
                    {
                        flg = false;
                        break;
                    }
                    ...
                    {
                        grp_fund -= abs(money[j] - rent[mid  - 1 - j]);
                    }
                }
            }
            if (flg)
