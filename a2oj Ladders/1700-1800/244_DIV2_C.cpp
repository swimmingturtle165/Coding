...
void dfs1(ll curr,vector<bool>& vst,vector<vector<ll>>& graph,stack<ll>& strg)
{
    ...
    {
        if(vst[graph[curr][i]]==false)
        {
            dfs1(graph[curr][i],vst,graph,strg);
        }
    }
    strg.push(curr);
}

void dfs2(ll curr,vector<bool>& vst,vector<vector<ll>>& graph,vector<ll>& cst,ll & freq,ll & mini)
{
    ...
    FOR(i,0,graph[curr].size())
    {
        if(vst[graph[curr][i]]==false)
        {
            dfs2(graph[curr][i],vst,graph,cst,freq,mini);
        }
    }
}
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        ll m;
        cin>>m;
        vector<vector<ll>> graph_curr(n,vector<ll>());
        vector<vector<ll>> graph_rev(n,vector<ll>());
        FOR(i,0,m)
        {
            ll a,b;
            cin>>a>>b;
        ...
        while (strg.size() >0)
        {
            ...
            else
            {
                ll mini=INT_MAX;
                ll fr=0;
                dfs2(strg.top(),vst,graph_rev,inp,fr,mini);
                ans+=mini;
                // ans%=MOD;
                ways=(ways*(fr%MOD))%MOD;
                strg.pop();
            }
