...
ll dfs( int curr,vector<vector<ll>> graph,vector<ll>& symbols,vector<bool>& vst)
{
    ll maxi=0,mini=0;
    vst[curr]=true;
    FOR(j,0,graph[curr].size())
    {
        if(vst[graph[curr][j]]==false)
        {
            dfs(graph[curr][j],graph,symbols,vst);
            maxi=max(maxi,symbols[graph[curr][j]]);
            mini=min(mini,symbols[graph[curr][j]]);
        }
    }
    symbols[curr]+=(maxi+mini);
    // cout<<curr<<"  "<<maxi<<" "<<mini<<" "<<symbols[curr]<<endl; 
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,n) cin>>symbols[i];
        ans=0;
        ans+=dfs(0,graph,symbols,vst);
        cout<<abs(symbols[0])<<endl;
    }
    return 0;
}
