...
vector<ll> prefixFn(string s)
{

    vector<ll> strg(s.size(), 0);
    FOR(i, 1, s.size())
    {
        int j = strg[i - 1];
        while (j != 0 && s[i] != s[j])
...
signed main(int argc, char **argv)
{
    ...
    while (t--)
    {
        ...
        {
            cnt=0;
            string str1=str.substr(0,v2)+'$'+str;
            strg=prefixFn(str1);
            FOR(i,0,strg.size())
            {
                if(strg[i]==v2)
