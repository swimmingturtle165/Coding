...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(m,1,pow(n,0.5)+1)
        {
            if(n%m==0)
            {
                int partition_size = n/m;
                int partitions = m;
                // cout << partition_size << " " << partitions << endl;
             
                
                FOR(j,0,partition_size)
                {
                    bool tmp = true;
                    FOR(i,0,partitions)
                    {
                        if(inp[j+i*partition_size]!=1)
                        {
                            tmp = false;
                            break;
                        }
