...
bool solve(ll curr,vector<ll>& vst,vector<ll>& status,vector<vector<ll>>& graph)
{
    // cout<<curr<<" ** ";
    bool flg=(status[curr]==false);
    bool tmp=false;
    vst[curr]=true;
    FOR(i,0,graph[curr].size())
    {
        ...
        if(vst[graph[curr][i]]==false)
        {
            if(tmp==false)
            {
                tmp=solve(graph[curr][i],vst,status,graph);
            }else{
                solve(graph[curr][i],vst,status,graph);
            }
        }
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<ll> vst(n+1,false);
        // FOR(i,1,n+1){
        //     dispvector<ll>(graph[i]);
        // }
        solve(1,vst,status,graph);
        cout<<strg.size()<<endl;
        for(auto it:strg)
        {
            cout<<it<<" ";
