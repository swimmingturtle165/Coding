...
int build(vector<ll>& inp,ll idx, ll level)
{
    if(inp[idx]==-1)
    {
        ll curr_level=build(inp,2*idx+1,level);
         curr_level=build(inp,2*idx+2,level);
        if(curr_level==1)
        {
            inp[idx]=(inp[2*idx+1] | inp[2*idx+2]);
        }
        else
        {
            inp[idx]=(inp[2*idx+1] ^ inp[2*idx+2]);
...
int update(int curr,int idx,int val,string path,int level,vector<ll>& inp)
{
    if(curr==path.size())
    {
        inp[idx]=val;
    ...
    {
        ll curr_level=update(curr+1,2*idx+1,val,path,level,inp);
    ...
    {
        ll curr_level=update(curr+1,2*idx+2,val,path,level,inp);
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<ll> inp(2*h-1,-1);

        FOR(i,0,h) cin>>inp[h-1+i];
        ...
        while(m--)
        {
            ...
            string path=binary_rep(idx-1);

            while(path.size()<n)
            {
                path="0"+path;
            }
            update(0,0,val,path,n,inp);
            cout<<inp[0]<<endl;
