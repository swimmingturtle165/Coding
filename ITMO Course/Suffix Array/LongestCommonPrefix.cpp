...
vector<ll> LCPArray(string str)
{
    ...
    FOR(i,0,n-1)
    {
        ...
        ll v2=p[v1-1];
        int st=k;
        while(i+k <n && v2+k<n && str[i+k]==str[v2+k])
        {
            k++;
        }
        // cout<<i<<" ## "<<v1<<" %% "<<v2<<" "<<st<<" => "<<k<<endl;
        lcp[v1]=k;
        k=max(k-1,0);
    }
   FOR(i,1,str.size())
   ...
   cout<<endl;
    // FOR(i,0,n)
    // {
    //     cout<<lcp[i]<<" "<<p[i]<<" "<<str.substr(p[i],n-p[i])<<endl;
    // }
    return lcp;
