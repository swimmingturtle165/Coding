...
vector<ll> suffix_array(string str)
{
    ll n=str.size();
    ll alphabet=256;
    vector<ll> p(n),c(n),cnt(max(alphabet+1,n+1),0);
    FOR(i,0,n)
    {
        cnt[str[i]]++;
    }
    ...
    FOR(h,0,v1)
    {
        ...
        FOR(i,1,n)
        {
            pair<ll,ll> curr={c[p[i]],c[(p[i]+(1<<h))%n]};
...
void leftMostBinarySearch(int start,int end,vector<ll>& suffixArray,string& str,ll& index,string &p)
{
    if(start>end)
    {
        return;
    ...
    int len=suffixArray.size()-suffixArray[mid];
    int len1=p.size();
    len=min(len,len1);
    if(p>str.substr(suffixArray[mid],len))
    {
        leftMostBinarySearch(mid+1,end,suffixArray,str,index,p);
    }
    else
    {
        if(p==str.substr(suffixArray[mid],len))
        {
            // cout<<"**  "<<suffixArray[mid]<<endl;
            index=min(mid,index);


        }
        
        leftMostBinarySearch(start,mid-1,suffixArray,str,index,p);
...
void RightMostBinarySearch(int start,int end,vector<ll>& suffixArray,string& str,ll& index,string &p)
{
    if(start>end)
    {
        return;
    ...
    if(p>str.substr(suffixArray[mid],len))
    {
        RightMostBinarySearch(mid+1,end,suffixArray,str,index,p);
    }
    else
    {
        if(p==str.substr(suffixArray[mid],len))
        ...
        {
        RightMostBinarySearch(start,mid-1,suffixArray,str,index,p);
...
ll countOccurances(string &str,string& p,vector<ll>&suffix_array)
{
    ll leftidx=INT_MAX;
    ll rightidx=INT_MIN;
    leftMostBinarySearch(0,str.size()-1,suffix_array,str,leftidx,p);
    RightMostBinarySearch(0,str.size()-1,suffix_array,str,rightidx,p);
    ll ans=0;
    ans=max(ans,rightidx+1-leftidx);
    return ans;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        string str1=str;
        str=str+'$';
        vector<ll> suffixArray=suffix_array(str);
        suffixArray.erase(suffixArray.begin());
        // dispvector<ll>(suffixArray);
        // cout<<"$$"<<endl;
        ll n;
        cin>>n;
        while(n--)
        {
            string p;
            cin>>p;
         cout<< countOccurances(str1,p,suffixArray)<<endl;
            // dispvector<ll>(indexes);
