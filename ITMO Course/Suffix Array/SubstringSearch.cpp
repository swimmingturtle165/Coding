...
void binarySearch(int start,int end,vector<ll>& suffixArray,string& str,vector<ll>& indexes,string &p)
{
    if(start>end)
    {
        return;
    ...
    {
        binarySearch(mid+1,end,suffixArray,str,indexes,p);
    }
    else
    {
        if(p==str.substr(suffixArray[mid],len))
        {
            // cout<<"**"<<endl;
            indexes.pb(suffixArray[mid]);
        }
        
        binarySearch(start,mid-1,suffixArray,str,indexes,p);
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        while(n--)
        {
            ...
            cin>>p;
            indexes.clear();
            binarySearch(0,suffixArray.size()-1,suffixArray,str1,indexes,p);
            if(indexes.size()>0)
            {
                cout<<"Yes"<<endl;
            }         
