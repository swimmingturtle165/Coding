...
struct segmenttree{
    ...
    ll get_segment(ll val,ll x,ll lx ,ll rx)
    {
        ...
        if(val<=left)
        {
            return get_segment(val, 2 * x + 1, lx, mid);
        }
        

        return     get_segment(val, 2 * x + 2, mid,rx);
    

        
    }
    
    ll get_segment(ll val)
    {
        return get_segment(val ,0, 0, sz);
    }
};
