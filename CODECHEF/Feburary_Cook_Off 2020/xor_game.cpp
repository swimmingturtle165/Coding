...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ll n;
        cin>>n;
        vector<pair<ll,ll>> fir_inp(n),sec_inp(n);
        ...
        if(n==1)
        {
            // cout<<fir_inp[0].first<<endl;
            cout<<sec_inp[0].first<<endl;
            continue;

        }
        sort(fir_inp.begin(),fir_inp.end());
        sort(sec_inp.begin(),sec_inp.end());

        bool flag=true;
        
        FOR(i,0,n)
        {
            if(fir_inp[i].first!=fir_inp[i].second)
            {
                flag=false;
                break;
            }
        ...
        if(flag)
        {
            ...
            FOR(i,0,n)
            {
                outp[sec_inp[i].second]=sec_inp[i].first;
            }
            FOR(i,0,n)
            {
                cout<<outp[i]<<" ";
