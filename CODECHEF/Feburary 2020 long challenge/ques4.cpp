...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        if(flag)
        {
            ...
            FOR(i,0,n)
            {
                ll val_req=p%sorted[i].first;
                if(val_req==0)
                {
                    val_req+=sorted[i].first;
                }
                FOR(j,0,n)
                {
                    if(i!=j && val_req%sorted[j].first!=0 && (val_req%sorted[j].first)<min(sorted[i].first,sorted[j].first))
                    {
                        falg2=false;
                        order[sorted[j].second]=(val_req/sorted[j].first)+1;
                        order[sorted[i].second]=(p-val_req)/sorted[i].first;
                        break;
