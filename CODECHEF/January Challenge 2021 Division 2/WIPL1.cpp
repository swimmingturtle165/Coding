...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        if(sum2<k)
        {
            ...
            FOR(i,1,cnt2+1)
            {
                strg[i]=strg[i-1]+strg[i];
            }
            FOR(i,0,cnt1+1)
            {
                FOR(j,cnt1+1,cnt2+1)
                {
                    ll tmp_sum1=sum1+inp[j]-inp[i];

                    ll tmp_sum2=sum2+inp[i]-inp[j];
                    
                    ll req=k-tmp_sum1;

                    bool flg=false;
                    ll extra=tmp_sum2-k;
                    ll idx=upper_bound(strg.begin(),strg.end(),extra)-strg.begin();
                    idx--;
                                     
                    
                    
                    ll idx1=lower_bound(strg.begin(),strg.end(),req)-strg.begin();
                    // cout<<tmp_sum1<<" "<<tmp_sum2<<" "<<extra<<" "<<req<<endl;
                    if(req<=0)
                    {
                        idx1=-1;
                    }
                    
                    if(idx<cnt2-j && tmp_sum2>=k && tmp_sum1>=k)
                    {
                     
                    mxi=max(mxi,idx-idx1);

                    flag=true;

                    cout<<mxi<<" "<<cnt2<<" "<<flag<<" "<<cnt2+1-mxi<<endl;
                    }
