...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        if(gs==2*k)
        {
            vector<vector<ll>> dp(n+1,vector<ll>(k+1,false));
            dp[0][0]=true;
            FOR(i,0,n)
            {
                FOR(j,0,k+1)
                {
                    if(dp[i][j] || (inp[i]<=j && dp[i][j-inp[i]]))
                    {
                        dp[i+1][j]=true;
                    }
                }
        ...
        else
        {
            ...
            FOR(i,0,idx1)
            {
                ...
                swap(inp[i+1],inp[idx]);
                
            }
            if(idx<n)
            {
            sort(inp.begin()+idx,inp.end());
            reverse(inp.begin()+idx,inp.end());

            }
            FOR(i,idx,n)
            {
                // cout<<sum2<<" "<<inp[i]<<endl;
