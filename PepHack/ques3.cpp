...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        for(int i=0;i<inp.size();i++)
        {
            ...
            events.push_back({st,-h,0});

            events.push_back({end,h,1});
        }
        ...
        for(int i=0;i<events.size();i++)
        {
            ...
            if(status==0)
            {
                strg.insert(curheight);
                if(maxi!=*strg.rbegin())
                {
                    // Update  newly found Max

                    maxi=*strg.rbegin();
                    ans.push_back({curlocation,maxi});
                }
            }
            else
            {
                auto x=strg.find(curheight);
                strg.erase(x);
                ...
                else if(maxi!=*strg.rbegin())
                {               
                   
                //    Update new maxi as current expires
