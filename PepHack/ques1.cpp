...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        double a1i,a2i,a1j,a2j,a1k,a2k;
        cin>>a1i>>a1j>>a1k;
        cin>>a2i>>a2j>>a2k;
        bool flg=false;
        double g=2;
        FOR(i,1,10001)
        {
            if(pow(x2-x1+((a2i-a1i)*((double)pow(i,2)))/g,2) +pow(y2-y1+((a2j-a1j)*((double)pow(i,2)))/g,2)+pow(z2-z1+((a2k-a1k)*((double)pow(i,2)))/g,2)<=pow(r1,2)+pow(r2,2))
            {
                flg=true;
                break;
            }
