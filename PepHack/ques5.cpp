...
int solve(int r,int c,vector<vector<char>>&graph,int n,vector<vector<ll>>& dp)
{
    if(r==-1)
    {
        return 0;
    ...
    if(r>=0 && c>=0 && r<n && c<n && graph[r][c]=='0')
    {
        return INT_MIN;
    }
    if(dp[r][c]!=INT_MIN)
    ...
    if(r==n)
    {
        ll v1=solve(r-1,c-1,graph,n,dp);
        ll v2=solve(r-1,c,graph,n,dp);
        ll v3=solve(r-1,c+1,graph,n,dp);
        


        dp[r][c]=max(dp[r][c],v1);
        dp[r][c]=max(dp[r][c],v2);
        dp[r][c]=max(dp[r][c],v3);
        ans=max(dp[r][c],ans);
    ...
    else
    {
        ...
        dp[r][c]=max(dp[r][c],v1+tmp);
        dp[r][c]=max(dp[r][c],v2+tmp);
        dp[r][c]=max(dp[r][c],v3+tmp);
        ans=max(dp[r][c],ans);
