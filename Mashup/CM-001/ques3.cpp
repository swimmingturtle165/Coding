...
ll dfs(ll curr,vector<vector<ll>> & graph,vector<bool>& vst,vector<ll>& children,vector<pll>& cnt1)
{
    ll currt=1;
    vst[curr]=true;
    for(auto v:graph[curr])
    {
        if(vst[v]==false)
        {
            currt+=dfs(v,graph,vst,children,cnt1);
        }
    }
    children[curr]=currt;
    ll v1=graph.size()-1;
    cnt1[curr]={currt*(v1-currt),curr};
    return currt;
