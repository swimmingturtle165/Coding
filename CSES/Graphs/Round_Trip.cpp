...
ll dfs(int curr,int parent,vector<vector<ll>>& graph,vector<bool>& vst)
{
    // cout<<curr<<endl;
    vst[curr]=true;
...
bool get_path(int curr,int parent,int trgt,vector<vector<ll>>& graph,vector<ll>& path,vector<bool>& vst)
{
    if(parent!=-1)
    {
        vst[curr]=true;
    ...
    for(auto u: graph[curr])
    {
        ...
        if(vst[u]==false && u!=parent)
        {
            // cout<<u<<endl;
            bool tmp=get_path(u,curr,trgt,graph,path,vst);
            if(tmp)
            {
                path.pb(curr);
                return true;
        ...
        FOR(i,1,n+1)
        {
            if(vst[i]==false)
            {
                ...
                if(tmp!=-1)
                {
                    ...
                    vector<ll> path;
                    get_path(tmp,-1,tmp,graph,path,vst);
