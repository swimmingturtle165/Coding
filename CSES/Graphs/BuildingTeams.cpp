...
bool dfs(ll curr, ll val,vector<ll>& strg, vector<vector<ll>>& graph)
{
    strg[curr]=val;
    ...
    for(auto u:graph[curr])
    {
        if(strg[u]==0 )
        {
            if(dfs(u,3-val,strg,graph)==false)
            {
                // cout<<u<<endl;
                return false;
            }
