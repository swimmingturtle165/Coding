...
void dfs(ll curr,vector<bool> &vst,vector<vector<ll>>& graph)
{
    if(vst[curr])
    {
        return;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<vector<ll>> graph1(n+1,vector<ll>());
        vector<vector<ll>> graph2(n+1,vector<ll>());
 
 
        vector<bool> conn1(n+1,false);
        vector<bool> conn2(n+1,false);
