...
ll solve(string &a,string &b,int i,int j,vector<vector<ll>>& dp)
{
    ...
    {
        dp[i][j]=1+solve(a,b,i,j+1,dp);
    }
    else if(j==b.size())
    {
        dp[i][j]=1+solve(a,b,i+1,j,dp);
    ...
    {
        dp[i][j]= min(solve(a,b,i+1,j,dp),min(solve(a,b,i,j+1,dp),solve(a,b,i+1,j+1,dp)))+1;
    }
    return dp[i][j];
}

ll solve2(string a,string b)
{
