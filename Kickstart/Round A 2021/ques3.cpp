...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        ll r,c;
        cin>>r>>c;
        vector<vector<ll>> strg(r,vector<ll>(c));
        ...
        while (pq.size()>0)
        {
            ...
            ll min_allowed=val-1;
            if(x>0 )
            {
                // top
                if(strg[x-1][y]<min_allowed)
                {
                ans+=max(0ll,min_allowed-strg[x-1][y]);
                strg[x-1][y]=min_allowed;
                pq.push(S(strg[x-1][y],x-1,y));
                }
            }
            if( y>0)
            {
                // left
                if(strg[x][y-1]<min_allowed)
                {
                ans+=max(0ll,min_allowed-strg[x][y-1]);
                strg[x][y-1]=min_allowed;
                pq.push(S(strg[x][y-1],x,y-1));
                }
            }
            if(x<r-1)
            {
