...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        ll r,c;
        cin>>r>>c;
        vector<vector<ll>> inp(r,vector<ll>(c));
        FOR(i,0,r)
        {
            FOR(j,0,c)
            {
        ...
        vector<vector<ll>> from_top(r,vector<ll>(c));
        vector<vector<ll>> from_left(r,vector<ll>(c));
        vector<vector<ll>> from_bottom(r,vector<ll>(c));
        vector<vector<ll>> from_right(r,vector<ll>(c));
        ...
        FOR(i,0,r)
        {
            FOR(j,0,c)
            {
                ll left=from_left[i][j];
                ll top=from_top[i][j];
                ll right=from_right[i][j];
                ll bottom=from_bottom[i][j];

                // left bottom
                ll v1=min(left/2,bottom);

                ans+=max(0ll,v1-1);
                // left top
                 v1=min(right/2,bottom);
                ...
                v1=min(left,bottom/2);

                ans+=max(0ll,v1-1);
                // left top
                 v1=min(right,bottom/2);
