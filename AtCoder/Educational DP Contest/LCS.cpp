...
void solve(string& s1,ll idx1,ll idx2,vector<vector<ll>>& dp,string& answ)
{

    if(idx1==0 || idx2==0) return;

    if(dp[idx1][idx2]>max(dp[idx1-1][idx2],dp[idx1][idx2-1]))
    {
        solve(s1,idx1-1,idx2-1,dp,answ);
        answ+=s1[idx1-1];
    }
    else
    {
        if(max(dp[idx1-1][idx2],dp[idx1][idx2-1])==dp[idx1][idx2-1])
        {
              solve(s1,idx1,idx2-1,dp,answ);
        }
        else
        {
              solve(s1,idx1-1,idx2,dp,answ);
        }
    }
}
