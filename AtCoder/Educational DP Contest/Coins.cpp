...
void solve(ll curr,ll heads,vector<ld>& arr,vector<vector<ld>>& dp)
{
    ...
    // i => heads
    // curr-i =>tails
    // dp[curr][i]=(dp[curr-1][i-1]*arr[i] + dp[curr-1][i]*(1-arr[i]))
    ld g=1;
    for(int i = 0 ; i<=curr;i++)
    {
        if(curr-1>=i)
        {
            dp[curr][i]=(dp[curr][i]+dp[curr-1][i]*(g-arr[curr-1]));
        }

        if(i>0)
        {
            // cout<<"**"<<endl;
            // cout<<dp[curr][i]<<" "<<dp[curr-1][i-1]<<" "<<arr[i]<<endl;
            // cout<<"**"<<endl;
            dp[curr][i]=(dp[curr][i]+dp[curr-1][i-1]*(arr[curr-1]));
        }
    }
    ll n=arr.size();
   
    solve(curr+1,curr+1,arr,dp);
}
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<vector<ld>> dp(n+2,vector<ld>(n+2,0));
        FOR(i,0,n+1)
        {
            dp[0][i]=0;
        }
