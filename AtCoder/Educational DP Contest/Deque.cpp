...
ll solve(ll lft,ll rgt,vector<ll>& arr,vector<vector<ll>>& dp)
{
     if(rgt<lft)
    {
        return 0;
    ...
    if(dp[lft][rgt]!=INT64_MIN)
    {
        return dp[lft][rgt];
    }
   dp[lft][rgt]=max(arr[lft]+min(solve(lft+1,rgt-1,arr,dp),solve(lft+2,rgt,arr,dp)),arr[rgt]+min(solve(lft+1,rgt-1,arr,dp),solve(lft,rgt-2,arr,dp)) );
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<ll> arr(n);
        ll sum=0;
        vector<vector<ll>> dp(n,vector<ll>(n,INT64_MIN));
        FOR(i,0,n) cin>>arr[i];
        FOR(i,0,n) sum+=arr[i];

       cout<<2*solve(0,n-1,arr,dp)-sum<<endl;
