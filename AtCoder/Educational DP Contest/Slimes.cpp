...
ll solve(ll start, ll end,vector<ll>& arr_sum,vector<vector<ll>>& dp)
{
    if(start==end)
    {
        return 0;
    ...
    ll v2=arr_sum[end]-(start==0?0:arr_sum[start-1]);
    
    ll mini=INT_MAX;
    FOR(i,start,end)
    {
        mini=min(mini,solve(start,i,arr_sum,dp)+solve(i+1,end,arr_sum,dp));
    }
    mini+=v2;
    dp[start][end]=mini;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        // dp [start][end] =min(dp[start+1][end],dp[start][end-1])+sum(arr(start .. end))
        ...
        cout<<solve(0,n-1,arr_sum,dp)<<endl;
