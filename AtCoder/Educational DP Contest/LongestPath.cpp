...
void solve(ll curr,vector<ll>& dist,vector<bool>& vst,vector<ll>& indegree,vector<vector<ll>>& graph)
{
    vst[curr]=true;
    ...
    {
        indegree[u]--;
        dist[u]=max(dist[u],dist[curr]+1);
        if(indegree[u]==0)
        {
            solve(u,dist,vst,indegree,graph);
        }
    }
}
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(i,1,n+1)
        {
            if(vst[i]==false && indegree[i]==0)
            {
                solve(i,dist,vst,indegree,graph);
            }
        }
