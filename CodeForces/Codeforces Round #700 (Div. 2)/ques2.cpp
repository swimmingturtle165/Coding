...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,n)
        {
            ll v1=(strg[i].second+a-1)/a;
            ans[i].first=v1*strg[i].first;
            ans[i].second=i;
            val-=ans[i].first;
        }
        // cout<<b<<" "<<val<<" "<<ans[n-1].second<<endl;
        sort(ans.begin(),ans.end());
        bool flg=false;
        FOR(i,0,n)
        {
            ll prev=val+b+ans[i].first;
            ll attack_power=strg[ans[i].second].first;
            ll health=strg[ans[i].second].second;
            ll v1=(health+a-1)/a;
            v1--;
            if(prev>=0 && (prev-v1*attack_power)>0)
            {
                flg=true;
            }
