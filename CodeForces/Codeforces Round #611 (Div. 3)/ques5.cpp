...
signed main(int argc, char** argv)
{
    ...
    unordered_multiset<ll,custom_hash> inp;
    
    unordered_set<ll,custom_hash> mini,maxi;
    ...
    sort(inpd.begin(),inpd.end());
    ll mini_cnt=0,maxi_cnt=0;
    ll prev=0;
    FOR(i,0,inpd.size())
    {
        ll curr=inpd[i];
        ...
        if(mini.count(curr-1)==0 && freq>0)
        {
            freq--;
            maxi_cnt++;
            mini.insert(curr-1);
        }
        if(mini.count(curr)==0 && freq>0)
        {
            mini.insert(curr);
            freq--;
            maxi_cnt++;
    ...
    {
        ll curr=inpd[i];
        ll freq=inp.count(curr);
         if(maxi.count(curr)==0 && maxi.count(curr-1)==0 &&maxi.count(curr+1)==0)
        {
            maxi.insert(curr+1);
            mini_cnt++;
        }
