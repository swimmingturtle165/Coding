...
vector<ll> GetsquareNumbers(ll n)
{
    vector<ll> ans(n+1);
    ll i=1;
    while(i*i<=n)
...
signed main(int argc, char** argv)
{
    ...
    long t=1;
    cin>>t;
    vector<ll> squareNumbers=GetsquareNumbers(1000000);
    while(t--)
    {
        ll n;
        cin>>n;
        ...
        FOR(i,0,n)
        {
            ll ans=inp[i]/squareNumbers[inp[i]];
            // cout<<ans<<" "<<inp[i]<<endl;
            strg[ans].push_back(inp[i]);
            a=max(a,(ll)strg[ans].size());

        }
        ll c=0;
        for(auto it:strg )
        {
            if(it.second.size()%2==0  ||  it.first==1)
            {
