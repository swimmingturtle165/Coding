...
ll timer=0;
vector<ll> entry_time;
vector<ll> exit_time;
vector<ll> depth;

vector<vector<pll>> strg;


void dfs(ll curr,ll par,ll dpth,vector<vector<ll>>& graph)
{
    timer++;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        entry_time=vector<ll>(n+1,0);
        exit_time=vector<ll>(n+1,0);
        depth=vector<ll>(n+1,0);
        strg=vector<vector<pll>>(n+1,vector<pll>());
        ...
        for(int i = 1;i<=n;i++)
        {
            strg[depth[i]].pb({entry_time[i],exit_time[i]});

        }
       for(int i = 1;i<=n;i++)
        {
            sort(strg[i].begin(),strg[i].end());
