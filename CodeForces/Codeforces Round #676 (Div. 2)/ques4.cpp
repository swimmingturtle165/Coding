...
void optimal( vector<vector<ll>>& arr)
{
    FOR(k,0,9)
    {
        FOR(r,0,3)
            ...
            FOR(c,0,3)
            {
                FOR(i,0,3)
                {
                    FOR(j,0,3)
                    {
                        ll y_req = r - i;
                        ll x_req = c - j;
                        if(x_req<2 && y_req<2 && x_req>-2 && y_req>-2)
                        {
                            arr[r][c] = min(arr[r][c], arr[i][j] + arr[1 + y_req][1 + x_req]);
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<vector<ll>> arr(3, vector<ll>(3,INT_MAX));
        arr[1][1] = 0;
        arr[0][0] = c4;
        arr[0][1] = c5;
        arr[1][0] = c3;
