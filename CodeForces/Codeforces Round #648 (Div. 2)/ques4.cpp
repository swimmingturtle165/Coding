...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        while (q.size()>0)
        {
            ...
            q.pop();
            if(x<0 || y<0 || x>=n || y>=m ||vst[x][y]==true || arr[x][y]=='#')
            {
                continue;
            }
            vst[x][y]=true;
            if(x>0 && arr[x-1][y]!='#' && vst[x-1][y]==false)
            {
                q.push({x-1,y});
            }
            if(y>0 && arr[x][y-1]!='#' && vst[x][y-1]==false)
            {
                q.push({x,y-1});
            }
            if(x<n-1 && arr[x+1][y]!='#' && vst[x+1][y]==false)
            {
                q.push({x+1,y});
            }
            if(y<m-1 && arr[x][y+1]!='#' && vst[x][y+1]==false)
            {
                q.push({x,y+1});
            }
