...
void solve(vector<vector<ll>>&graph)
{
    ll ans=INT_MAX;
    ll n=graph.size()-1;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        // vector<vector<ll> > strg(n,vector<ll> (65,0));
        vector<ll> inp(n);
        vector<vector<ll> > strg(65);
        bool flg=false;
        FOR(i,0,n)
        {
            cin>>inp[i];  
        ...
        else
        {
            ...
            FOR(i,0,65)
            {
                FOR(j,0,strg[i].size())
                {
                    FOR(k,j+1,strg[i].size())
                    {
                        graph[strg[i][j]].pb(strg[i][k]);
                    }
                }
            }
            solve(graph);
