...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ll n,k,z;
        cin>>n>>k>>z;
        vector<ll> inp(n),prefix_sum(n);
        cin>>inp[0];
        prefix_sum[0]=inp[0];
        FOR(i,1,n)
        {
            cin>>inp[i];
            prefix_sum[i]=prefix_sum[i-1]+inp[i];
        }
        ll ans=inp[0];
        FOR(i,1,k+1)
        {
            ...
            ll kt=k+1-2*lft;
            kt=min(kt,n-1);
            cout<<i<<" "<<lft<<" "<<kt<<" "<<prefix_sum[kt]+lft*(prefix_sum[i]+prefix_sum[i-1])-prefix_sum[i]<<endl;
            ans=max(ans,prefix_sum[kt]+lft*(prefix_sum[i]+prefix_sum[i-1])-prefix_sum[i]);
