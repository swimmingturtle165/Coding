...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ll r, n;
        cin >> r >> n;
        vector<vector<ll>> inp(n, vector<ll>(3,0));
        FOR(i,0,n)
        {
        ...
        for (int i = n - 2; i >= 0;i--)
        {
            for (int j = i + 1; j < n;j++)
            {
                ...
                if(inp[j][0]-inp[i][0]>=abs(inp[j][1]-inp[i][1])+abs(inp[j][2]-inp[i][2]))
                {
                    dp[i] = max(dp[i], dp[j] + 1);
                }
                if(j-i>2*r)
                {
                    dp[i] = max(dp[i],1 + maxi[j]);
        ...
        FOR(i,0,n)
        {
            if(abs(0-inp[i][0])>=abs(inp[i][1]-1)+abs(inp[i][2]-1))
            {
                ans = max(ans, dp[i]);
            }
        }
