...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        string str1,str2;
        cin>>str1>>str2;
        unordered_map<char,vector<ll>> indexes;
        vector<ll> cnt1(1000,0),cnt2(1000,0);
        vector<bool> unvis(1000,false);
        FOR(i,0,str1.size())
        {
            indexes[str1[i]].pb(i+1);
        }
        ll ans=INT_MIN;
        vector<ll> lft(str2.size(),0);
        vector<ll> rgt(str2.size(),0);

        ll prev=-1;
        FOR(i,0,str2.size())
        {
            while(indexes[str2[i]][cnt1[str2[i]]]<=prev)
            {
            cnt1[str2[i]]++;

            }
            lft[i]=indexes[str2[i]][cnt1[str2[i]]];
            prev=lft[i];
        }
        prev=n+1;
        FORDE(i,str2.size()-1,0)
        {
            cnt2[str2[i]]++;

            while(indexes[str2[i]][indexes[str2[i]].size()-cnt2[str2[i]]]>=prev)
            {
            cnt2[str2[i]]++;

            }
            rgt[i]=indexes[str2[i]][indexes[str2[i]].size()-cnt2[str2[i]]];
            prev=rgt[i];
        }
        ...
        FOR(i,0,str2.size()-1)
        {
            ans=max(ans,lft[i+1]-lft[i]);
            ans=max(ans,rgt[i+1]-rgt[i]);
            ans=max(ans,rgt[i+1]-lft[i]);
            ans=max(ans,lft[i+1]-rgt[i]);
