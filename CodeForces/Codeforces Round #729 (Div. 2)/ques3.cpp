...
signed main(int argc, char** argv)
{
    ...
    long t=1;
    cin>>t;
    vector<ll> gt{2,3,2,3,2,4,2,3,2,3,2,5};
    while(t--)
    {
        ll n;
        cin>>n;
        ...
        while(curr<=n)
        {
            ...
            // while(gcd(curr,idx+1ll)==idx+1ll)
            // {
            //     // return 0;
            //     idx++;
            // }
            curr=(curr*(idx1+1ll))/gcd(curr,idx1+1ll);
            ll num_val=(n/curr);
            num_val%=MOD;
            ll v3=(idx+1ll-prev)%MOD;
            // cout<<curr<<" "<<num_val<<" "<<prev<<" "<<idx<<" "<<idx1<<" ";
            answ+=((num_val*(v3)));
            // cout<<answ<<endl;
            answ%=MOD;
            idx1++;
