...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        map<ll,ll> strg;
        unordered_map<ll,vector<ll>> order;
        FOR(i,0,n) 
        {
             ll tmp;
            cin>>tmp;
        ...
        for(auto it: strg)
        {
            if(prefix_sum<it.first)
            {
                ans=it.second;
                anw.clear();
                anw.push_back(it.first);
            }
            else
            {
                ans+=it.second;
        ...
        FOR(i,0,anw.size())
        {
            FOR(j,0,order[anw[i]].size())
            {
               strg1[curr]=order[anw[i]][j];
               curr++;
            }
        }
