...
vector<pll> getFactorization(int n)
{
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<ll> vst(n,false);

        // vector<vector<pll>> strg(n);
        // FOR(i,0,n)
        // {
        //     strg[i]=getFactorization(inp[i]);
        // }
        ...
        while (pq.size()>0)
        {
            ...
            bool flg=false;
            while (lft>=0 && find(parent,idx,lft)==false)
            {
                bool tmp=true;

                tmp=gcd(val,inp[lft])==val;

                if(tmp)
                {
                    ans+=val;
                    weighted_union(parent,sz,idx,lft);
                    lft--;
                }
                else
                {
            ...
            while (rgt<n && find(parent,idx,rgt)==false)
            {
                bool tmp=true;
                unordered_map<ll,ll> strg1;

                tmp=gcd(val,inp[rgt])==val;

                if(tmp)
                {
                    ans+=val;
                    weighted_union(parent,sz,idx,rgt);
                    rgt++;
                }
                else
                {
        ...
        FOR(i,0,n)
        {
            if(find(parent,0,i)==false)
            {
                ans+=p;
                weighted_union(parent,sz,0,i);
            }
        }
        cout<<ans<<endl;
