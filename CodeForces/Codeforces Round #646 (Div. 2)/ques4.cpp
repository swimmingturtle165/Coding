...
bool find(vector<ll>&Arr,int A,int B)
{
   ...
   vector<ll> cst,initial,final;

pair<ll,ll> solve(ll curr, vector<unordered_set<ll>>& graph,vector<bool>& vst)
{
    pair<ll,ll> ans1={0,0};
    vst[curr]=true;
    for(auto&it:graph[curr])
    {
        if(vst[it]==false)
        {
           pair<ll,ll> tmp= solve(it,graph,vst);
           ans1.first+=tmp.first;
           ans1.second+=tmp.second;
        }
    }
    if(initial[curr]!=final[curr])
    {
        ans1.first++;
        ans1.second=min(cst[curr]*ans1.first,cst[curr]+ans1.second);
    }
    else
    {
        ans1.second=min(ans1.second,ans1.first*cst[curr]);
    }
    cout<<curr<<" "<<ans1.first<<" "<<ans1.second<<endl;
    return ans1;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(i,1,n+1)
        {
            cin>>cst[i]>>initial[i]>>final[i];
            // cout<<i<<endl;
        }
        vector<unordered_set<ll>> graph(n+1,unordered_set<ll>());
        ...
        vector<unordered_set<ll>> tmp=graph;
        while(q.size()>0)
        {
            ll curr=q.front();
            q.pop();
            ...
            for(auto&it:tmp[curr])
            {
                if(tmp[it].count(curr)!=0)
                {
                tmp[it].erase(tmp[it].find(curr));
                }
        ...
        cout<<solve(prev,graph,vst).second<<endl;
