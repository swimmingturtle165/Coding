...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        cin>>str;
        int n=str.size();
        vector<int> prefix_ones(n+1,0),prefix_zero(n+1,0);
        ...
        FOR(i,0,n)
        {
            ...
            else 
            {
                ...
                prefix_ones[i+1]=1+prefix_ones[i];

            }
        }

        ll total_ones=prefix_ones[n];
        ll total_zero=prefix_zero[n];
        prefix_zero.push_back(total_zero);
        prefix_ones.push_back(total_ones);

        ll ans=min(total_ones,total_zero);
        // FOR(i,0,n+2)
        // {
        //     cout<<prefix_zero[i]<<" ";
        // }
        ...
        {
                // cout<<ans<<" **  ";
                ll v1=(total_zero-prefix_zero[i])+prefix_ones[i-1];
                ll v2=(total_ones-prefix_ones[i])+prefix_zero[i-1];
                // cout<<v1<<" "<<v2<<endl;
