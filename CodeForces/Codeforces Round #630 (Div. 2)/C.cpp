...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        if(k%2==1)
        {
            ...
            vector<ll> maxi((k+1)/2,0);
            vector<vector<ll>> strg((k+1)/2,vector<ll>(26,0));
            FOR(i,0,(k+1)/2)
            {
                FOR(j,0,n/k)
                {
                    ...
                    if(i+1!=(k+1)/2)
                    {
                    strg[i][str[j*k +k-1-i]-'a']++;                        
                    maxi[i]=max(maxi[i],strg[i][str[j*k +k-1-i]-'a']);
                    
                    }
                    maxi[i]=max(maxi[i],strg[i][str[j*k +i]-'a']);
        ...
        else
        {
            ...
            vector<vector<ll>> strg((k)/2,vector<ll>(26,0));
            vector<ll> maxi((k)/2,0);
