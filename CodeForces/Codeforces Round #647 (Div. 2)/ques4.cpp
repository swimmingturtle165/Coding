...
bool helper(int curr,vector<ll>& topics,vector<vector<ll>>& graph,vector<bool>& vst_nodes,vector<bool>& vst_topics )  
{
    if(vst_topics[topics[curr]])
    {
        return false;
    }
    
    vst_nodes[curr]=true;
    vst_topics[topics[curr]]=true;

    FOR(i,0,graph[curr].size())
    {
        if(vst_nodes[graph[curr][i]]==false)
        {
            bool flg=helper(graph[curr][i],topics,graph,vst_nodes,vst_topics);
            if(flg==false)
            {
                return false;
            }
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(i,1,n+1) cin>>topics[i];

        bool check=true;
        vector<vector<ll>> colors(n+1,vector<ll>());
        FOR(i,1,n+1)
        {
            colors[topics[i]].pb(i);
        ...
        for(auto it:colors[i])
        {
            ...
            for(auto neighbour:graph[it])
            {
                if(topics[neighbour]<topics[it])
                {
                    strg.insert(topics[neighbour]);
                }
                if(topics[neighbour]==topics[it])
                {
                    check=false;
