    ...
    while(t--)
    {
        ...
        else
        {
            ...
            while(idx<n-1)
            {
                ...
                FOR(i,0,sec)
                {
                    if(vst[i]==false && strg1.count(curr+inp[i])>0)
                    {
                        tmp=i;
                        break;
                    }
                ...
                if(tmp!=-1)
                {
                    ...
                    vst[tmp]=true;
                    strg[2*idx]=inp[sec];
                    strg[2*idx+1]=inp[tmp];
                    while(inp[sec]!=curr+inp[tmp])
                    {
                        sec++;
                    }
                    curr=inp[sec];
            ...
            strg[2*idx+1]=inp[idx1];

            cout<<"YES"<<endl;
             cout<<strg[2*n-1]+strg[2*n-2]<<endl;
            for(int i=2*n-1;i>=1;i=i-2)
            {
                cout<<strg[i]<<" "<<strg[i-1]<<endl;
            }
