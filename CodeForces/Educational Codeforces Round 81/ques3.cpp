...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        ll curr=0,ans=1;
        bool flg=true;
        for(ll i = 0 ; i <trg.size();i++)
        {
           
             if(arr[0][trg[i]-'a']==INT_MAX)
            {
                flg=false;
                break;
            }
            else if(arr[curr][trg[i]-'a']==INT_MAX)
            {
                 curr=arr[0][trg[i]-'a']+1;
                ans++;
            }
            else
            {
                curr=arr[curr][trg[i]-'a']+1;
                if(curr>=org.size() &&i!=trg.size()-1)
                 {
                curr=0;
                ans++;
                 }
