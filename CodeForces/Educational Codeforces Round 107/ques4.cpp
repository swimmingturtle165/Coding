...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        while (curr.size()!=n)
        {
            ...
            pll tmp =*min_element(strg[prv].begin(),strg[prv].end());

            for(auto it:strg[prv])
            {
                if(it.f==tmp.f && strg2[it.s]<strg2[tmp.s])
                {
                tmp.f=it.f;
                tmp.s=it.s;
                }
            ...
            // cout<<tmp.f<<" "<<(char)('a'+tmp.s)<<" "<<prv<<endl;
