...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        int n2=str2.size();
        // cout<<n1<<" "<<n2<<endl;
        vector<pair<int,int>> strg(n1+2,{n2,n2});
        ...
        FORDE(i,n1-1,0){
            ...
            // backtrack in next step dp[i]=dp[i+2]

            // match i.e str1[i]==str2[dp[i+1]-1]  dp[i]=dp[i+1]-1 
            
            strg[i]=strg[i+2];

            if( str1[i]==str2[strg[i+1].f-1])
            {
                strg[i].second=min(strg[i].second,strg[i+1].f-1);
            }
            if( str1[i]==str2[strg[i+1].s-1])
            {
                strg[i].second=min(strg[i].second,strg[i+1].s-1);
            }

            if(min(strg[i].f,strg[i].s)==0)
            {
                flg=true;
                break;
            }
