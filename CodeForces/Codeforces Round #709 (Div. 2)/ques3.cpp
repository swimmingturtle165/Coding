...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        cin>>n>>m;
        lft.assign(m,0);
        vector<vector<ll>> inp(n+1,vector<ll>());
        ...
        ll k2=ceil((double(m)/(double(2))));
        FOR(i,0,n+1)
        {
            sort(inp[strg[i].second].begin(),inp[strg[i].second].end(),cmp);
            FOR(j,0,inp[strg[i].second].size())
            {
                ll v1=inp[strg[i].second][j];
                if(order[v1]==-1 && count[strg[i].second]+1<=(m+1)/2)
                {
                    order[v1]=strg[i].second;
                    count[strg[i].second]++;
                }
                lft[v1]--;
            }
