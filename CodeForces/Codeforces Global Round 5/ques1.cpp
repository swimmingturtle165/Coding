...
signed main(int argc, char** argv)
{
    ...
    for(int i = 0 ; i <n;i++)
    {
    // cout<<odd_neg<<" "<<odd_pos<<" "<<arr[i]<<" "<<endl;

        if(arr[i]%2==0 || (arr[i]>0 &&odd_neg%2==0)||(arr[i]<0 &&odd_pos%2==0))
        {
            cout<<arr[i]/2<<endl;
            if(arr[i]%2!=0 && arr[i]>0) odd_neg++;
            else if(arr[i]%2!=0 && arr[i]<0) odd_pos++;
        }
        else 
        {
            cout<<(arr[i]/2)+(arr[i]>0?1:-1)<<endl;
            if(arr[i]>0)
            {
                odd_neg++;
            }
