...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        {
            cin >> inp[i].first;
            mini = min(mini, inp[i].first);
            sum += inp[i].first;

            inp[i].second = i + 1;
            if(inp[i].first>=(W+1ll)/(ll)2 && inp[i].first<=W && flg==false)
            {
                flg = true;
                idx.pb(i + 1);
            }
        ...
        else 
        {
            ...
            ll run_sum = 0;
            ll id = 0;
            while(run_sum<(W+1)/2 && id<n)
            {
                if(run_sum+inp[id].first>W)
                {
                    id++;
                    continue;
                }
                run_sum += inp[id].first;
                idx.pb(inp[id].second);
                id++;
            }
            if(run_sum<(W+1)/2 || run_sum>W)
            {
                cout << -1 << endl;
            }
            else
