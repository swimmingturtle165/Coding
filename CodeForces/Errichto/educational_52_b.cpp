    ...
    while(t--)
    {
        ...
        FOR(i,0,n)
        {
            if(str[i]=='G')
            {
                ...
                while(i<n && str[i]=='G' )
                {
                    strg[strg.size()-1].second=i;
                    i++;
                }
            }
        }
        ll ans=0;
        bool flg=strg.size()>2;
        bool flg2=strg.size()>1;
        FOR(i,0,strg.size())
        {
            ll len1=strg[i].second-strg[i].first;
            len1++;
            ans=max(ans,len1+flg2);
            if(i>0 && str[strg[i].first-2]=='G')
            {
                ll len2=strg[i-1].second-strg[i-1].first;
                 len2++;
                 ans=max(ans,len1+len2+flg);
            }
        }
        cout<<ans<<endl;
    }
