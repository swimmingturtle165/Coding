...
ll solve(vector<pair<ll,ll>> &ans,int n)
{
    vector<vector<ll>> dp(n+1,vector<ll>(n+1,INT_MAX));
    FOR(i,0,ans.size())
    {
        dp[ans[i].first][ans[i].second]=1;
        dp[ans[i].second][ans[i].first]=1;
    }
    FOR(i,0,n)
    {
        dp[i+1][i+1]=0;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        reverse(lmt.begin(),lmt.end());
        vector<ll> degree_grt_than_one;
        bool flg=true;
        vector<pair<ll,ll>> ans;
        vector<ll> done(n+1,false);
        FOR(i,0,lmt.size()-1)
        {
            ans.pb({lmt[i].second,lmt[i+1].second});
            
            lmt[i].first--;
            lmt[i+1].first--;
            
            done[lmt[i].second]=true;
            done[lmt[i+1].second]=true;
       ...
       ll j=0;
        FOR(i,1,done.size())
        {
            if(done[i] == false && (j>=lmt.size()-1 || lmt[j].first<=0))
            {
                flg=false;
                break;
            }
            else if(done[i]==false)
            {
                 ...
                 ans.pb({i,lmt[j].second});
                 lmt[j].first--;
                 if(j<lmt.size()-1 && lmt[j].first<=0)
                 {
                 j++;
                 }
            }
