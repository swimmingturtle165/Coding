...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ll n,m;
        cin>>n>>m;
        vector<ll> curr_temp(n),limit_temp(n),fine(n),cost(n);
        
        FOR(i,0,n) {
            cin>>curr_temp[i]>>limit_temp[i]>>fine[i]>>cost[i];
            }

        ll ans=0;
        FOR(i,0,n)
        {
            if(limit_temp[i]-curr_temp[i]>=m)
            {
                // cout<<ans<<" "<<cost[i]<<endl;
                ans+=cost[i];
            }
            else if(limit_temp[i]>curr_temp[i])
            {
                ll cap=limit_temp[i]-curr_temp[i];
                ans+=min((cost[i]*((m+cap-1)/cap)),cost[i]+(m)*fine[i]);
            }
            else
            {
                ans+=cost[i]+(m)*fine[i];
