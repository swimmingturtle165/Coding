...
// ***************GLOBAL*****************
void solve(int idx,string curr,vector<bool> mismatch,vector<string>& arr)
{
    if(idx==arr[0].size())
    {
        flg1=true;
    ...
    else
    {
        ...
        {
            if(vst[arr[i][idx]-'a']==false)
            {
                vst[arr[i][idx]-'a']=true;
                bool flg=true;
                for(int j=0;j<arr.size();j++)
                {
                    if(mismatch[j] && arr[j][idx]!=arr[i][idx])
                    {
                        flg=false;
                        break;
                    }
                ...
                if(flg)
                {
                    ...
                    solve(idx+1,curr+arr[i][idx],tmp,arr);
                }
                if(flg1)
                {
                    return;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<bool> mismatch(n,false);
        solve(0,"",mismatch,strg);
        if(flg1)
        {
            cout<<ans<<endl;
        }
