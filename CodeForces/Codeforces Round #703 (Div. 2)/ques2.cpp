...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ll n;
        cin>>n;
        vector<ll> xcord(n),ycord(n);
        FOR(i,0,n) cin>>xcord[i]>>ycord[i];

        sort(xcord.begin(),xcord.end());
        sort(ycord.begin(),ycord.end());
        if(n==2)
        {
            ll ans=abs(1+xcord[1]-xcord[0])*abs(1+ycord[1]-ycord[0]);
            cout<<ans<<endl;
        }
        else if(n%2==0)
        {
            ...
            ll idx1=v;
            ll idx2=v-1;
             ll ans=abs(1+xcord[idx1]-xcord[idx2])*abs(1+ycord[idx1]-ycord[idx2]);
