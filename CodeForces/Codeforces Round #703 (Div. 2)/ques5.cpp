...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<vector<ll>> strg(n+1,vector<ll>(n+1,INT_MAX));
        FOR(i,1,n+1)
        {
            FOR(j,0,graph[i].size())
            {
                FOR(k,0,graph[graph[i][j].first].size())
                {
                    strg[i][graph[i][j].first]=min(strg[i][graph[i][j].first],graph[i][j].second+graph[graph[i][j].first][k].second);
                }
            }
        }
