...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ll n;
        cin>>n;
        vector<ll> inp(n),prefix(n),suffix(n);
        FOR(i,0,n)
        {
            cin>>inp[i];
        }
        ...
        FOR(i,1,n)
        {
            prefix[i]=__gcd(prefix[i-1],inp[i]);
        }
        for(int i=n-2;i>=0;i--)
        {
            suffix[i]=__gcd(suffix[i+1],inp[i]);
        }
        ll ans=suffix[1];
        FOR(i,1,n)
        {
            if(i==n-1)
            {
                ans*=prefix[i-1]/__gcd(prefix[i-1],ans);
            }
            else
            {
                ans*=(__gcd(prefix[i-1],suffix[i+1])/(__gcd(prefix[i-1],__gcd(suffix[i+1],ans))));
            }
