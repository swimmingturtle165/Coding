...
signed main(int argc, char** argv)
{
    ...
    {
           ll n,k;    
        ll min_moves=0,max_moves=0;
        string str;
        cin>>n>>k;
        cin>>str;
        deque< vector<ll> > req_moves;
        bool flg=true;
    while(flg)
    {
        vector<ll> curr_moves;
        curr_moves.clear();
        FOR(i,0,str.size())
        {
        ...
        {
            for (int i : curr_moves) 
            {
                swap(str[i], str[i - 1]);
            }
            min_moves++;
            req_moves.pb(curr_moves);
        }
        
    }
    if(k>max_moves ||k<min_moves)
    {
        cout<<-1<<endl;
    }
    // else if(min_moves==k)
    ...
    else
    {
        ...
        while(diff--)
        {
            cout<<1<<" "<<req_moves.front().back()<<endl;
            req_moves.front().pop_back();
            if(req_moves.front().size()==0)
            {
                req_moves.pop_front();
                diff++;
            }
