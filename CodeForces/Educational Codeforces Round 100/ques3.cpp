...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        inp.push_back({INT64_MAX,0});
        ll ans=0;
        ll curr_time=inp[0].first;
        ll start=0;
        ll end_pt=inp[0].second;
        ll end_time=inp[0].first+abs(start-end_pt);
        if(end_time<=inp[1].first)
                  ans++;
        
        FOR(i,1,n)
        {
            if(inp[i].first<end_time)
            {
                if(start <= inp[i].second && inp[i].second <= end_pt)
                {
                    ll tmp=curr_time+abs(inp[i].second-start);
                        if(inp[i].first <=tmp && tmp<=inp[i+1].first)
                        {
                            ans++;
                        }
                }
                else if(start >= inp[i].second && inp[i].second >= end_pt)
                {
                     ll tmp=curr_time+abs(inp[i].second-start);
                        if(inp[i].first <=tmp && tmp<=inp[i+1].first)
                        {
            ...
            {
                  start=end_pt;
                  curr_time=max(end_time,inp[i].first);
                  end_pt=inp[i].second;
                  end_time=curr_time+abs(start-end_pt);
                  if(end_time<=inp[i+1].first)
                  ans++;
            }
        }
        cout<<ans<<endl;
