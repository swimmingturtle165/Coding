...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        cin>>n>>d;
        vector<ll> inp(n);
        vector<ll> strg(d,INT64_MAX);
        FOR(i,0,n) cin>>inp[i];
        ll curr=0;
        ll idx=0;
        
        while(strg[curr]==INT64_MAX)
        {
            ll f=10*idx+d;
            ll rem=f%d;
            strg[rem]=min(f,strg[rem]);
            f+=10;
            curr=f%d;
            idx++;
        }
        ...
        FOR(k,0,d)
        {
            ...
            FOR(i,0,d)
            {
                FOR(j,0,d)
                {
                    strg[(i+j)%d]=min(strg[(i+j)%d],strg[i]+strg[j]);
                }
            }
