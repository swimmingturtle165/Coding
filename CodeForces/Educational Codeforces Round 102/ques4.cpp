...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        string str;
        cin>>str;
        vector<ll> strg(n+2,0);
        ll val=0;
        vector<ll> left_maxi(n+1,0),left_mini(n+1,0);
        vector<ll> right_maxi(n+2,0),right_mini(n+2,0);
        ...
        FOR(i,0,n)
        {
            if(str[i]=='+')
            {
                val++;
                strg[i+1]=val;
                left_maxi[i+1]=max(left_maxi[i],val);
                left_mini[i+1]=min(left_mini[i],val);
            }
            else
            {
                val--;
        ...
        strg[n+1]=strg[n];
        right_maxi[n+1]=strg[n];
        right_mini[n+1]=strg[n];
        left_mini[0]=0;
        left_maxi[0]=0;
        ...
        FORDE(i,n,1)
        {
            right_mini[i]=min(strg[i],right_mini[i+1]);
            right_maxi[i]=max(strg[i],right_maxi[i+1]);
        }
        ...
        while(m--)
        {
            ...
            ll diff=strg[right]-strg[left-1];
            ll mxt=max(left_maxi[left-1],right_maxi[right+1]-diff);
            ll mnt=min(left_mini[left-1],right_mini[right+1]-diff);
