...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        unordered_map<ll,unordered_set<ll>> strg_boy,strg_girl;
        ll a,b,k;
        cin>>a>>b>>k;
        vector<pll> inp(k);
         FOR(i,0,k)
         ...
         FOR(i,0,k)
         {
             strg_boy[inp[i].first].insert(inp[i].second);
             strg_girl[inp[i].second].insert(inp[i].first);
         }
         ll ans=0;
         FOR(i,0,k)
         {  
             ll v1=strg_boy[inp[i].first].size();
             ll v2=strg_girl[inp[i].second].size();
