...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<ll> imp_apps(imp);
        vector<ll> normal_apps(n-imp);
        ...
        FOR(i,0,n)
        {
            if(inp[i].first==2)
            {
                imp_apps[j]=inp[i].second;
                j++;
            }
            else
            {
                normal_apps[h]=inp[i].second;
                h++;
            }
            
        }
        sort(imp_apps.begin(),imp_apps.end(),mycmp);
        sort(normal_apps.begin(),normal_apps.end(),mycmp);

        FOR(i,1,imp_apps.size())
        {
            imp_apps[i]+=imp_apps[i-1];
        }
        FOR(i,1,normal_apps.size())
        {
            normal_apps[i]+=normal_apps[i-1];
        }
        ll ans=INT_MAX;
        ll temp=0;
        if(lower_bound(normal_apps.begin(),normal_apps.end(),m)!=normal_apps.end())
        {
            ll idx=lower_bound(normal_apps.begin(),normal_apps.end(),m)-normal_apps.begin();
            ans=min(ans,idx+1);
        }
        FOR(i,0,imp_apps.size())
        {
            temp=imp_apps[i];
            if(imp_apps[i]>=m)
            {
           ...
           else if(lower_bound(normal_apps.begin(),normal_apps.end(),m-temp)!=normal_apps.end())
           {
            ll idx=lower_bound(normal_apps.begin(),normal_apps.end(),m-temp)-normal_apps.begin();
            ans=min(ans,2*(i+1)+idx+1);
