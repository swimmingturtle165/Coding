...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        unordered_map<ll,unordered_set<ll>> strg;
        unordered_map<ll,vector<ll>> strg1;
        int val=0;
        FOR(i,0,n)
        {
            if(strg[val+1].count(arr1[i])==0)
            {
                strg1[val+1].pb(arr1[i]);
                strg[val+1].insert(arr1[i]);
        ...
        int mini=INT_MAX;
        for(auto it:strg1) mini=min(mini,(int)it.s.size());

        // cout<<mini<<endl;

        for(auto &it:strg1){
            while ((int)it.s.size()!=mini)
            {
                it.s.pop_back();
            }
        }

        unordered_map<ll,vector<ll>> strg2;
        for(auto it:strg1)
        {
            for(auto it1:it.s)
            {
                strg2[it1].push_back(it.f);
            }
        }

        FOR(i,0,n){
            if(strg2[arr[i]].size()>0){
                cout<<strg2[arr[i]].back()<<" ";
                strg2[arr[i]].pop_back();
            }
            else{
                cout<<0<<" ";
            }
