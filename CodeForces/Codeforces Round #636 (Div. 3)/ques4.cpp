...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        vector<ll> inp(n);
        FOR(i,0,n) cin>>inp[i];
        ordered_set_pair strg,strg2;
        unordered_map<ll,ll,custom_hash> mp;
        FOR(i,0,n/2)
        {
            strg.insert({min(inp[i],inp[n-1-i]),i});
            strg2.insert({max(inp[i],inp[n-1-i]),i});

            ll v=mp[inp[i]+inp[n-1-i]];
            v++;
            mp[inp[i]+inp[n-1-i]]=v;
        }
        ll ans=n;
        FOR(i,2,2*k+1)
        {
            ll tmp=i;
            ll tmp_ans=(n/2)-mp[tmp];
            // cout<<tmp_ans<<endl;
            // cout<<tmp<<"**"<<endl;

            tmp_ans+=(n/2)-strg.order_of_key({tmp-1,INT_MAX});
            tmp_ans+=strg2.order_of_key({tmp-k-1,INT_MAX});
            // cout<<tmp_ans<<endl;
            ans=min(ans,tmp_ans);
        }
        cout<<ans<<endl;
