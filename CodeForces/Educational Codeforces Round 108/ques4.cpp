...
ll solve(ll left,ll right,bool flg,vector<ll>& a1,vector<ll>& a2)
{
    ...
    if(flg && dp2[left][right]!=-1)
    {
        return dp2[left][right];
    }
    else if(flg==false && dp1[left][right]!=-1)
    {
        return dp2[left][right];
    }
    // 4 options
    
    // left+1 right-1 with reversal
    // left right-1 
    // left+1 right-1
    ll v1=a1[left]*a2[right]+a2[left]*a1[right]+solve(left+1,right-1,true,a1,a2);
    
    dp2[left][right]=v1;
    

    dp1[left][right]=max(v1,dp1[left][right]);

    if(flg==false)
    {
        ll v2=a1[right]*a2[right]+solve(left,right-1,false,a1,a2);
        ll v3=a1[left]*a2[left]+solve(left+1,right,false,a1,a2);

        dp1[left][right]=max(dp1[left][right],max(v2,v3));
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        dp1.assign(n+1,vector<ll>(n+1,-1));
        dp2.assign(n+1,vector<ll>(n+1,-1));

        cout<<solve(0,n-1,false,inp1,inp2)<<endl;
