...
pair<ll,ll> solve(ll curr,ll parent, vector<vector<ll>>& graph, vector<ll>& mini,vector<ll>& maxi)
{
    ll ans1=0;
    ll ans2=0;
    ...
    for(auto u:graph[curr])
    {
        if(u!=parent)
        {
            pll tmp=solve(u,curr,graph,mini,maxi);

            ans1+=max(tmp.f+abs(maxi[curr]-mini[u]),tmp.s+abs(maxi[curr]-maxi[u]));

            ans2+=max(tmp.s+abs(mini[curr]-maxi[u]),tmp.f+abs(mini[curr]-mini[u]));

        }
    }
    // cout<<curr<<" "<<ans1<<" "<<ans2<<endl;
    return {ans2,ans1};
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        pll tmp= solve(1,-1,graph,mini,maxi);
        cout<<max(tmp.f,tmp.s)<<endl;
