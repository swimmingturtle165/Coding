...
void solve(ll start,ll end,vector<pll>& moves)
{
    if(end<=2)
    {
        return;
    ...
    // now we push from (v1+1 ro end-1 ) divided by end
    // then we do end/v1 and again by v1
    // then call (start,v1)
    FOR(i,v1+1,end)
    {
        moves.pb({i,end});
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,moves.size()) cout<<moves[i].f<<" "<<moves[i].s<<endl;
