...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        for(int i = 1 ; i <n-1;i++){
            ...
            if((inp[i].second< inp[i-1].second && inp[i].second<inp[i+1].second) ||(inp[i].second > inp[i-1].second && inp[i].second>inp[i+1].second))
            {
                flg=true;
                idx.push_back(i);
            }
        ...
        else
        {
            if(flg)
            {
                ...
                // cout<<idx.size()<<endl;
                if(idx.size()==1){
                    ans=abs(inp[0].second-inp[n-1].second);
                }
                for(int i = 0 ; i <idx.size();i++)
                {

                    ll tmp=idx[i];
                    // cout<<"%% &&"<<tmp<<" "<<inp[tmp].second<<endl;
                    if(inp[tmp].second <inp[tmp-1].second && inp[tmp].second < inp[tmp+1].second)
                    {
                    ll v2=inp[tmp-1].second;
                    ll v3=inp[tmp+1].second;
                    ll v4=2*min(abs(inp[tmp].second-inp[tmp-1].second),abs(inp[tmp].second-inp[tmp+1].second));
                   
                    // cout<<v4<<" %%"<<endl;
                   
                    if(abs(inp[tmp].second-inp[tmp-1].second)>abs(inp[tmp].second-inp[tmp+1].second))
                    {
                        inp[tmp-1].second=inp[tmp].second;
                        // v4+=abs(inp[tmp].second-inp[tmp+1].second);
                    }
                    else
                    {
                        inp[tmp+1].second=inp[tmp].second;
                    ...
                    // }
                    // cout<<endl;
                    ll v1=v4+abs(inp[tmp].second-inp[0].second)+abs(inp[tmp].second-inp[n-1].second);
                    // cout<<v1<<" %%"<<endl;
                    inp[tmp-1].second=v2;
                    inp[tmp+1].second=v3;
                    ans=min(ans,v1);
            ...
            {
                cout<<min(abs(inp[0].second-inp[n-2].second),abs(inp[1].second-inp[n-1].second))<<endl;
            }
        }
