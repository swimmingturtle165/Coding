...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(j,0,30)
        {
            ...
            FOR(i,0,n)
            {
                ll l = lower_bound(tmp.begin()+i+1, tmp.end(), (1 << j) - tmp[i]) - tmp.begin();
			    ll r = lower_bound(tmp.begin()+i+1, tmp.end(), (1 << (j+1)) - tmp[i]) - tmp.begin();
		    	cnt += r - l;
 
			    ll v = lower_bound(tmp.begin()+i+1, tmp.end(), (1 << (j+1)) + (1 << j) - tmp[i]) - tmp.begin();
			    cnt += n - v;
            }
            ans += (cnt % 2) *1LL* (1 << j);
            // cout<<j<<endl;
