...
signed main(int argc, char** argv)
{
    ...
    ll n,m,prev=0;
    cin>>n>>m;
    vector<ll> inp(n,-1),sort_inp,sum_arr(n+1,0);
    for(ll i = 0 ; i <n;i++)
    {
        cin>>inp[i];
        sum_arr[i+1]=prev+inp[i];
        prev=sum_arr[i+1];
    }

    ll curr=0,curr_sum;
    unordered_multiset<ll> done;
    for(ll i = 0;i<n;i++)
    {
        
        if(m-sum_arr[i]>=inp[i])
        {
            cout<<0<<" ";
        }
        else
        {
            ...
            ll cnt=0;
            bool flg=true;
            for(int j = 100 ; j >=1 &&flg==true;j--)
            {
                if(done.count(j)>0)
                {
                   ll ct=done.count(j);
                   ll ter= (inp[i]-(m-sum_arr[i]+maxi)+(j-1))/j;
                   if(ter<=ct)
                   {
                       cnt+=ter;
                       cout<<cnt<<" ";
