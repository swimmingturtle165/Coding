...
bool getFactorization(ll x,unordered_set<string>& strg) 
{ 
    vector<ll> ret; 
    while (x != 1) 
    { 
    ...
    while(idx<ret.size())
    {
        ...
        ll val=ret[idx];
        idx++;
        while(idx<ret.size() && ret[idx]==ret[idx-1])
        {
            idx++;
            cnt++;
        }
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        unordered_set<string> strg;

        ll cnt=1;
        bool flg=true;
        FOR(i,0,n)
        {
            bool res=getFactorization(inp[i],strg);
            if(res==true )
            {
                flg=false;
                cnt++;
                strg.clear();
                getFactorization(inp[i],strg);
            }
            else
            {
                flg=true;
