...
void insert(vector<ll>& strg,ll &ans,ll idx,ll n)
{
    if(idx==0 || idx==n+1)
    {
        return;
    }
    if(strg[idx-1]>strg[idx] && strg[idx]<strg[idx+1])
    {
        ans-=strg[idx];
    }
    if(strg[idx-1]<strg[idx] && strg[idx]>strg[idx+1])
...
void erase(vector<ll>& strg,ll &ans,ll idx,ll n)
{
    if(idx==0 || idx==n+1)
    {
        return;
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        while (q--)
        {
            ...
            {
                erase(inp,ans,left-1,n);
                erase(inp,ans,left,n);
                erase(inp,ans,left+1,n);

                if(right-1!=left+1 && right-1!=left)
                erase(inp,ans,right-1,n);
                
                if(right-1!=left)                
                erase(inp,ans,right,n);

                erase(inp,ans,right+1,n);
                
                swap(inp[left],inp[right]);
                
                insert(inp,ans,left-1,n);
                insert(inp,ans,left,n);
                insert(inp,ans,left+1,n);
                
                if(right-1!=left+1 && right-1!=left)
                insert(inp,ans,right-1,n);
                
                if(right-1!=left)
                insert(inp,ans,right,n);
