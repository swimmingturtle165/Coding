...
signed main(int argc, char** argv)
{
    ...
    vector<vector<ll>> graph(n+1),graph1(n+1);
    ...
    {
        ll minimal_intersection=INT_MAX;
        FOR(i,1,n+1)
        {
            FOR(j,i+1,n+1)
            {
                unordered_set<ll> v;
                FOR(k,0,graph1[i].size())
                {
                    v.insert(graph1[i][k]);
                }
                 FOR(k,0,graph1[j].size())
                {
                    v.insert(graph1[j][k]);
                }
                minimal_intersection=min(minimal_intersection,(ll)graph1[i].size()+graph1[j].size()-v.size());
            }
        }
       ll ans=m-minimal_intersection;
       cout<<ans<<endl;
    }
