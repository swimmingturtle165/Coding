...
signed main(int argc, char** argv)
{
    ...
    ll n;
    cin>>n;
    vector<ll> maxi_g,mini_g;
    for(ll i = 0 ; i <n;i++)
    {
        ll tmp;
        cin>>tmp;
        ll mini=INT_MAX,maxi=INT_MIN,prev;
        bool flag=false;
        for(ll j = 0 ; j <tmp;j++)
        {
            ll temp;
    ...
    ll cnt=0;
    sort(maxi_g.begin(),maxi_g.end());
    sort(mini_g.begin(),mini_g.end());
    ll prev=0;
    for(ll i = 0 ; i <mini_g.size();i++)
    {
        for(ll j = prev ; j <maxi_g.size();j++)
        {
            if(mini_g[i]>=maxi_g[j])
            {                
                prev++;
            }
            else
