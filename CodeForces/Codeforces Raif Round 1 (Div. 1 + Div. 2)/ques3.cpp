...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        ll n = str.size();
        ll ans = n;
        vector<pair<ll, char>> strg;
        vector<pair<ll, char>> strgh;
        strg.clear();
        // cout << str << endl;
        FOR(j,0,n)
        {
        ...
        for (int i = strg.size() - 1; i >= 0;i--)
        {
            if(strg[i].second=='B')
            {
                if(i>0)
                {
                    ll mini=min(strg[i].first,strg[i-1].first);
                    strg[i].first -= mini;
                    strg[i - 1].first -= mini;
                    ans -= 2* mini;
                    if(i>1)
                    {
                        strg[i - 2].first += strg[i].first;
                        strg[i].first = 0;
                    }
                    // FOR(j,0,strg.size())
                    // {
                    //     cout << strg[j].second << " " << strg[j].first<<endl;
                    // }
                    // cout << "******" << ans << endl;
                }
            }
        ...
        FOR(i,0,strg.size())
        {
            if(strg[i].second=='A' && strg[i].first>0)
            {
                rem = 0;
            }
            else if(strg[i].second=='B')
            {
                ll v = (rem + strg[i].first) / 2;
                ll tmp = (rem + strg[i].first) % 2;
                rem = tmp;
                ans -= 2 * (v);
            }
