...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ll n;
        cin>>n;
        vector<ll> a_i(n),b_i(n);
        ordered_set strg;
        ll ans=0;
        unordered_multiset<ll,custom_hash> str;
        vector<ll> dups;
        FOR(i,0,n) cin>>a_i[i];
        FOR(i,0,n) {
            cin>>b_i[i];
            ans+=strg.order_of_key((a_i[i]-b_i[i]));
            FOR(j,0,dups.size())
            {
                if(dups[j]<a_i[i]-b_i[i])
                {
                    ans+=(str.count(dups[j])-1);
                }
                
                
            }
            if(str.count(b_i[i]-a_i[i])==1)
            {
                dups.pb(b_i[i]-a_i[i]);
                sort(dups.begin(),dups.end());
            }
            str.insert(b_i[i]-a_i[i]);
            strg.insert(b_i[i]-a_i[i]);

               

            // cout<<ans<<" "<<b_i[i]-a_i[i]<<endl;
        }
        cout<<ans<<endl;
    }
    return 0;
