...
signed main(int argc, char** argv)
{
    ...
    FOR(i,0,n)
    {
       ...
       {
           prefix[i]=INT_MAX;
        //    suffix[n-1-i]=inp[n-1-i];
       }
       else
       {
           prefix[i]=(prefix[i-1] &(~inp[i-1]));
        //    suffix[n-1-i]=(suffix[n-i]&(~inp[n-i-1]));
    ...
    // dispvector<ll>(prefix);
    // dispvector<ll>(suffix);
    ...
    FOR(i,0,n)
    {
          ...
          ll  tem=((inp[i] &((prefix[i])))&((suffix[i])));
