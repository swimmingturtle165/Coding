...
ll solve(ll i,ll j,vector<ll>& ones,vector<ll>& zeros,vector<vector<ll>>& dp )
{
    ...
    ll v1=min(solve(i,j+1,ones,zeros,dp) , abs(ones[i]-zeros[j])+ (solve(i+1,j+1,ones,zeros,dp)));
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
         ...
         cout<<solve(0,0,ones,zeroes,dp)<<endl;
    }
    return 0;
}
