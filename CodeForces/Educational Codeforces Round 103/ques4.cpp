...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        string str;
        cin>>n>>str;
        vector<ll> left_n(n+1,0);
        vector<ll> left_inv(n+1,0);
        vector<ll> right_n(n+1,0);
        vector<ll> right_inv(n+1,0);

        FOR(i,0,n)
        {
            if(str[i]=='L')
            {
                left_n[i+1]=1+left_inv[i];
                left_inv[i+1]=0;
            }
            else
            {
                left_inv[i+1]=1+left_n[i];
                left_n[i+1]=0;
            }
        ...
        FORDE(i,n-1,0)
        {
            if(str[i]=='R')
            {
                right_n[i]=1+right_inv[i+1];
                right_inv[i]=0;
            }
            else
            {
                right_inv[i]=1+right_n[i+1];
                right_n[i]=0;
