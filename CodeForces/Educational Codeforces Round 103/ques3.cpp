...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        cin>>n;
        vector<ll> lengths(n);
        vector<pll> connectivity(n);

        FOR(i,0,n) cin>>lengths[i];

        FOR(i,0,n) cin>>connectivity[i].first; 

        FOR(i,0,n) cin>>connectivity[i].second; 
        FOR(i,0,n)
        {
            if(connectivity[i].first>connectivity[i].second)
            {
                swap(connectivity[i].first,connectivity[i].second);
            }
        }

        vector<ll> continuing_cycle_left(n,0);

        vector<ll> continuing_cycle_right(n,0);

        
        vector<bool> cycle_formed(n,false);
        FOR(i,1,n)
        {
            if(connectivity[i].first==connectivity[i].second )
            {
                continuing_cycle_left[i]=(2+lengths[i]-1+connectivity[i].second-connectivity[i].first);
            }
            else
            {
                continuing_cycle_left[i]=max(2+lengths[i]-1+connectivity[i].second-connectivity[i].first,continuing_cycle_left[i-1]+2+lengths[i]-1-(connectivity[i].second-connectivity[i].first));
            }
            
            
        }
        // dispvector<ll>(continuing_cycle_left);
        cout<<*max_element(continuing_cycle_left.begin(),continuing_cycle_left.end())<<endl;
