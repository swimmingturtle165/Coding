...
signed main(int argc, char** argv)
{
    ...
    sort(third.begin(),third.end(),mycmp);
    ll curr=r;
    bool flg=true;
    ll i =0,j=0;
    while(j<third.size())
    {
        debug2(third[j].first,third[j].second);
        if(r<third[j].first)
        {
            while(r<third[j].first && i<first.size() &&r>=first[i].first)
            {
                r+=first[i].second;
                i++;
            }
        ...
        else if((i!=first.size() && r+third[j].second>=first[i].first) || (i==first.size() &&))
        {
            r+=third[j].second;
            j++;
        }
