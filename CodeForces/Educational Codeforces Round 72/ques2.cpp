...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        else if(sensible.size()==0)
        {
            cout<<-1<<endl;
        }
        else
        {
            sort(sensible.begin(),sensible.end(),mycmp);
            ll diff1=sensible[0].first-sensible[0].second;
            
            ll temp=x-max_damage;
            temp+=(diff1-1);
            ll ans=temp/diff1;
            ans++;
            cout<<ans<<endl;
