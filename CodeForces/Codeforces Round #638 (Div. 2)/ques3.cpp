    ...
    while(t--)
    {
        ...
        else
        {
            ll end=k;
             ll j=1;
             string ans="";
            while(end*j<=n &&str[(j-1)*k]==str[(j*k)-1] &&((str[(j*k)-1]==str[0] && j==1) ||str[k]==str[str.size()-1]))
            {
                ans+=str[(j*k)-1];
                j++;
            }
            // cout<<str[(j-1)*k]<<" "<<j<<endl;
            if(end*(j-1)<n)
            {
            
                if(str[(j-1)*k]!=str[str.size()-1])
                {


                   ans+=str.substr((j-1)*k,n-((j-1)*k));
                }
                else
                {
                    ll left=n-((j-1)*k);
