...
signed main(int argc, char** argv)
{
    ...
    ll n,l;
    cin>>n>>l;
    vector<pair<ld,pair<ll,ll>>> strg(n);
    ...
    for(ll i = 0 ; i <n;i++)
    {
        ...
        strg[i]={(ld) temp/(ld) curr,{curr,temp}};
        inp[i]={temp,curr};
         curr*=2;
    }
    sort(strg.begin(), strg.end(), [] (const pair<ld,pair<ll,ll>> &left, const pair<ld,pair<ll,ll>> &right) {
    ...
    // for(ll i = 0 ; i <strg.size();i++)
    // {
    //     cout<<strg[i].first<<"  "<<strg[i].second.first<<endl;
    // }
    ll val=0;
    ll ans=0;
    ll final_ans=INT_FAST64_MAX;
    for(ll i =0;i<n;i++)
    {
        if(l>=strg[i].second.first)
        {
            ll v=l/strg[i].second.first;
            ll cost=v*strg[i].second.second;
            l=l%strg[i].second.first;
            ans+=cost;
            if(l!=0)
            final_ans=min(ans+strg[i].second.second,final_ans);
            if(l==0)
