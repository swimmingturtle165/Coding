...
ll dfs(ll curr,ll parent,vector<unordered_set<ll>>& graph,vector<ll>& answ)
{   
   ...
   for(auto it :graph[curr])
   {
       ...
       int val=dfs(it,curr,graph,answ);
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
       ...
       ll val= dfs(start,-1,graph,answ);

        cout<<answ.size()/4<<endl;

        for(int i = 0 ; i <answ.size();i+=4)
        {
            cout<<answ[i]<<" "<<answ[i+1]<<" "<<val<<" "<<answ[i+3]<<endl;
            val=answ[i+2];
        }
