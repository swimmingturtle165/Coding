...
signed main(int argc, char** argv)
{
    ...
    vector<ordered_set> strg(26);
    string str;
    cin>>str;
    ll n=str.size();
    FOR(i,0,n)
    {
        strg[str[i]-'a'].insert(i+1);
    }
    ll q;
    cin>>q;
    while(q--)
    {
        ...
        if(a==1)
        {
            ...
            cin>>pos>>ch;
            pos--;
            strg[str[pos]-'a'].erase(pos+1);
            strg[ch-'a'].insert(pos+1);
            str[pos]=ch;
        }
        else
        {
           ...
           for(ll i = 0 ; i <26;i++)
           {
               ...
               ll val=(strg[i].order_of_key(right+1)-strg[i].order_of_key(left));
               if(val>0)
               {
                   ans++;
               }
