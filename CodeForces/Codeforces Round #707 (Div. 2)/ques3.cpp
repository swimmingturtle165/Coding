...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,n)
        {
            FOR(j,i+1,n)
            {
                if(strg.find(inp[i].first+inp[j].first)!=strg.end())
                {
                    pll tmp = strg[inp[i].first + inp[j].first];

                    if(tmp.second!=j && tmp.second!=i && tmp.first!=j && tmp.first!=i)
                    {
                        // cout << inp[i].first << " " << inp[j].first << endl;
                        // cout << inp[tmp.first].first << " " << inp[tmp.second].first << endl;

                        ans.pb(inp[tmp.first].second);
                        ans.pb(inp[tmp.second].second);
                        ans.pb(inp[i].second);
                        ans.pb(inp[j].second);
                        flg = true;
                        break;
                ...
                strg[inp[i].first + inp[j].first] = {i, j};
            }
            if(flg)
            {
                break;
