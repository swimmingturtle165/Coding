...
void printpaths(int start,int end,vector<bool>&trav,string ans)
{   
    if(start==end)
    {
        ans+=(char)(start+'0');
        cout<<ans<<endl;//" "<<start<<endl;
        return ;
    }  
  ...
  for(int i=0;i<graph[start].size();i++)
  {
      if(trav[graph[start][i].nbr]==false)
      {
          printpaths(graph[start][i].nbr,end,trav,ans+ (char)(start+'0')+" -> ");        
      }        
  } 
  trav[start]=false;    
...
int main()
{
  ...
  cout<<ans<<endl;
  vector<bool> trav1(7,false);
  //trav1.resize(7);
  printpaths(start,end,trav1,"");
}