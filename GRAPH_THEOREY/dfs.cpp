...
void createGraph(int x,int y,vector<vector<int>>&graph)
{
    graph[x].pb(y);
}
void dfs(int curr,vector<bool> &visited,vector<vector<int>>&graph,string curr_path)
{
    ...
    else
    {
        ...
        {
            if(visited[graph[curr][i]]==true)
            dfs(graph[curr][i],visited,graph,curr_path);
        }
    }    
}
